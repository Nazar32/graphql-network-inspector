...
describe('networkHelpers.getRequestMatchConfidence', () => {
  const URL = 'http://example.com/graphql'

  const buildWebRequest = (
    body: string
  ): DeepPartial<chrome.webRequest.WebRequestBodyDetails> => ({
    url: URL,
    method: 'POST',
    requestBody: {
      raw: [{ bytes: new TextEncoder().encode(body) }],
  ...
  const buildNetworkRequest = (
    body?: string,
    bodySize = -1
  ): DeepPartial<chrome.devtools.network.Request> => ({
    request: {
      url: URL,
      method: 'POST',
      headers: [],
      bodySize,
      ...(body === undefined ? {} : { postData: { text: body } }),
    },
  })

  const run = (
    networkRequest: DeepPartial<chrome.devtools.network.Request>,
    webRequest: DeepPartial<chrome.webRequest.WebRequestBodyDetails> | null
  ) =>
    getRequestMatchConfidence(
      networkRequest as any,
      webRequest as any,
  ...
  it('reports "body" when the request bodies are identical', async () => {
    const body = JSON.stringify({ query: 'query Foo { user }' })
    const confidence = await run(
      buildNetworkRequest(body),
      buildWebRequest(body)
    )
    expect(confidence).toBe('body')
  })

  it('reports "none" for a different url', async () => {
    const body = JSON.stringify({ query: 'query Foo { user }' })
    const confidence = await run(buildNetworkRequest(body), {
      ...buildWebRequest(body),
      url: 'http://other.com/graphql',
    })
    expect(confidence).toBe('none')
  })

  it('reports "none" for a different method', async () => {
    const body = JSON.stringify({ query: 'query Foo { user }' })
    const confidence = await run(buildNetworkRequest(body), {
      ...buildWebRequest(body),
      method: 'GET',
  ...
  it('reports "operation" when the bodies differ but the operation is the same', async () => {
    // Another extension can rewrite the variables of a request in flight
    const webRequest = buildWebRequest(
      JSON.stringify({
        query: 'query Foo { user }',
        variables: { id: 1 },
      })
    )
    const networkRequest = buildNetworkRequest(
      JSON.stringify({
        query: 'query Foo { user }',
        variables: { id: 2 },
      })
    )
    const confidence = await run(networkRequest, webRequest)
    expect(confidence).toBe('operation')
  })

  it('reports "none" when the bodies hold different operations', async () => {
    const webRequest = buildWebRequest(
      JSON.stringify({ query: 'query Foo { user }' })
    )
    const networkRequest = buildNetworkRequest(
      JSON.stringify({ query: 'query Bar { account }' })
    )
    const confidence = await run(networkRequest, webRequest)
    expect(confidence).toBe('none')
  })

  it('reports "size" when the devtools body is missing but the size matches', async () => {
    // Chrome 152 and later can leave postData out of the devtools entry
    const body = JSON.stringify({ query: 'query Foo { user }' })
    const size = new TextEncoder().encode(body).byteLength
    const confidence = await run(
      buildNetworkRequest(undefined, size),
      buildWebRequest(body)
    )
    expect(confidence).toBe('size')
  })

  it('reports "endpoint" when the devtools body and size are both missing', async () => {
    const body = JSON.stringify({ query: 'query Foo { user }' })
    const confidence = await run(
      buildNetworkRequest(undefined),
      buildWebRequest(body)
    )
    expect(confidence).toBe('endpoint')
  })

  it('reports "none" when there is no web request', async () => {
    const confidence = await run(buildNetworkRequest('{}'), null)
    expect(confidence).toBe('none')
  })
})

describe('networkHelpers.getNetworkRequestStartTime', () => {
  it('reads the start time of a request', () => {
    const details = {
      startedDateTime: '2026-09-23T10:00:00.000Z',
    } as chrome.devtools.network.Request
    expect(getNetworkRequestStartTime(details)).toBe(
      Date.parse('2026-09-23T10:00:00.000Z')
    )
  })

  it('returns undefined when the start time is missing', () => {
    const details = {} as chrome.devtools.network.Request
    expect(getNetworkRequestStartTime(details)).toBeUndefined()
  })

  it('returns undefined when the start time cannot be read', () => {
    const details = {
